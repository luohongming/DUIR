...
def build_dataloader(dataset, dataset_opt, num_gpu=1, dist=False, sampler=None, seed=None):
    """Build dataloader.  # 构建数据加载器

    Args:
        dataset (torch.utils.data.Dataset): Dataset.
        dataset_opt (dict): Dataset options. It contains the following keys: # 数据集选线
            phase (str): 'train' or 'val'.  # 阶段
            num_worker_per_gpu (int): Number of workers for each GPU.   # 每个GPU的工作数量
            batch_size_per_gpu (int): Training batch size for each GPU.  # 每个GPU的训练batch大小
        num_gpu (int): Number of GPUs. Used only in the train phase.   # 仅在训练的时候使用
            Default: 1.
        dist (bool): Whether in distributed training. Used only in the train
            phase. Default: False.
        sampler (torch.utils.data.sampler): Data sampler. Default: None.  # 数据采样器
